...
- `hypertrophy/avatar/` — `MuscleAvatar` (SVG-Bodymap), `OverviewAvatarPanel` (CC, Volumen-Tracker: Avatar färbt Muskelgruppen nach Volumen, darunter **Balken/Text-Toggle** mit collapsed-Default (Höhe ~180px, Gradient-Fade + „Alle anzeigen"-Button) und expand-on-demand; Avatar+Hover unverändert), `WorkoutAvatarPanel`, `MuscleExerciseList`, `anatomy-paths.ts` (SVG-Pfade)
- `endurance/` — `KmGraphSection` (CC, Recharts AreaChart wöchentliches Volumen), `RunCalendar` (CC, Adaption des Hypertrophy-Kalenders mit Double-Day-Indikator), `MetricsDashboard` (CC, sieben klickbare Tiles mit Detail-Popovern: RHR, HRV, Sleep, Race-Predictions, Training Status, VO₂, Lactate Threshold), `TrainingsSection` (RSC, zwei Cards für Empfohlen/Historisch), `TrainingZoneCalculator` (CC, EINE konsolidierte Übersicht — kompakte Card am Ende von `/endurance`. Friel-Modell für Running, LTHR-verankert: Z1 &lt; 85 %, Z2 85–89, Z3 90–94, Z4 95–99, Z5 ≥ 100 % LTHR. Datenquellen werden in `zone-estimation.ts` automatisch kombiniert (Garmin-LT2 + Pace↔HF-Regression + Best-20-min); Pace pro Zone kommt aus der Regression, Z5 aus Intervall-Work-Splits. Methodik &amp; Datenbasis + Details pro Zone liegen in Popovern, nicht inline), `PlanSetupForm` (CC, Phase 4: Form mit Live-Pace-Berechnung, Pace-Zone-Auto-Ableitung mit Manual-Override, Datei-Upload PDF **oder Bild**), `PerformanceDashboard` (legacy, nicht mehr benutzt — bei Cleanup entfernen)
...
| `utils/nutrition-recommendation.ts` | `buildNutritionRecommendation({...,tags})` — deterministische Kalorien-Anpassung je Phase. **Cheat-Tag-Override:** `effectiveCaloriesForDay()` + `buildEffectiveDays()` ersetzen den fddb-Wert sobald ein Cheat-Day/Cheat-Meal-Tag vorliegt: kcalTarget → direkter Override; sonst fddb ×1,5 (Cheat-Day) bzw. ×1,25 (Cheat-Meal); Cheat-Day ohne Tracking → ausgeschlossen + Annotation. Empfehlung selbst: Ziel-Rate (Cut −0,5 %/Wo, Bulk +0,25 %/Wo, Maintenance 0) vs. beobachtete Rate, 7700-kcal-Regel, gerundet auf 50, gedeckelt ±500. Wird auch im KI-Kontext (`dashboard/context.ts`) verwendet, damit Empfehlung und KI dieselbe „wahre" Intake-Reihe sehen. |
...
| `endurance/zone-estimation.ts` | `estimateZonesFromRuns(runs, from, to, {garminLtPaceSecPerKm, lthr})` — konsolidierte Friel-Zonen (Z1 &lt; 85 % LTHR, Z2 85–89, Z3 90–94, Z4 95–99, Z5 ≥ 100). Drei Pace-Quellen werden zur **konsolidierten Schwellen-Pace** kombiniert (Median): Garmin LT2, Pace↔HF-Regression der Steady-Splits @ LTHR, Best-20-min-Pace. `buildZoneRows()` leitet pro Zone Pace-Range ab — bei vorhandener Regression direkt via `paceAtHr()` für die HF-Grenzen, sonst über Friel-Faktoren auf die Schwellen-Pace; Z5-Pace aus Intervall-Work-Splits (kumulativ ≥ 5 min). Pure, client-safe. Mindest-Datenlage Regression: ≥ 10 Punkte aus ≥ 3 Steady-Läufen, HF-Spanne ≥ 15 bpm, Steigung > 0. |
